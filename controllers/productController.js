...
// Главная страницы - список товаров
async function listPage(req, res) {
    const { Product } = getModels();
    const products = await Product.findAll({ order: [['id', 'ASC']] });
    res.render('index', { title: 'Clothing Store', products });
}
...
async function create(req, res) {
    ...
    await Product.create({ name, category, desc, price, pic });
    res.redirect('/');
}
...
async function update(req, res) {
    ...
    const product = await Product.findByPk(req.params.id);
    if (!product) return res.status(404).send('Not found');
    await product.update({ name, category, desc, price, pic });
    res.redirect('/');
}
